...
class _MIDNIGHT(Action_AbstClass):
    ...
    def action(self):
        # 襲撃対象の一覧を取得
        # 循環importのため無理矢理 type(self.player_.role)はwolfのはず
        p_list = [p.player_name for p in self.master_.global_object.players_alive]
        ...
        while True:
            ...
            if data.isdigit() and (int(data) in [1, 2, 3]):
                priority_ = int(data)
                self.player_.role.try_attack = attack_player  # 誰を襲撃しようとした人物を保存
                self.master_.submit_answer(
                    submit_type="attack", user=attack_player, priority=priority_)  # 選択を登録
                ok_send = self.player_.send_data(
                    f"submit: {attack_player}を優先度{priority_}で襲撃\n")
                return
            else:
                ok_send = self.player_.send_data(
                    "1,2,3を入力してください!\n> ", with_CR=False)
