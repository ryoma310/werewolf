...
class PlayerThread(Thread):
    ...
    def check_start(self):
        ...
        if self.master_.global_object.started:
            ok_send = self.send_data(
                "the game has already started, please wait and reconnect in next game.")
            return True
        else:
            return False
            ...
            if self.master_.global_object.player_num is None:
                ...
                while True:
                    ...
                    if data.isdigit() and int(data) > 0:
                        ...
                        ok_send = self.send_data(
                            f"Thank you, please wait for {num} players ready...\n")
                        break
                    else:
                        ok_send = self.send_data(
                            "please enter positive digit number\nHow many players? > ", with_CR=False)
    ...
    def ask_name(self):
        ...
        while True:
            ...
            if not success:
                ok_send = self.send_data(
                    f"{name_} is not available. please use another\nenter your name > ", with_CR=False)
                continue  # もう一回

            ok_send = self.send_data(
                f"Thank you, {name_}! please wait for game to begin...\n")
            break
    ...
    def select_role(self):
        ...
        while True:
            ...
            if data.isdigit() and (int(data) in role_dict_.keys()):
                role_ = getattr(classes.roles, role_dict_[int(data)].name.lower(
                )).player_instance(self.player_name, self, self.master_)
                self.role = role_
                self.listen_broadcast = True
                self.master_.wait_answer_done()
                ok_send = self.send_data(
                    f"受理しました, あなたは {role_.role_name} です.\n")
                break
            else:
                ok_send = self.send_data(
                    "役職の番号を入力してください.\n役職選択 > ", with_CR=False)
        ...
        while True:
            ...
            if data.isdigit() and (int(data) in p_dict.keys()):
                ...
                self.master_.submit_answer(
                    submit_type="vote", user=voted_player)
    ...
    def run(self):
        ...
        while not self.master_.global_object.end_flag:
            # 朝
            self.send_data(
                f"player: {self.master_.global_object.day} day morning")
            if self.alive:
            ...
            if self.end_flag:
                break
            self.send_data(
                f"player: {self.master_.global_object.day} day daytime")
            ...
            if self.end_flag:
                break
            self.send_data(
                f"player: {self.master_.global_object.day} day midnight")
            if self.alive:
                self.role.get_knowledge(TIME_OF_DAY.MIDNIGHT)
                self.role.take_action(TIME_OF_DAY.MIDNIGHT)
                self.master_.wait_answer_done()
