...
class _MIDNIGHT(Action_AbstClass):
    ...
    def action(self):
        ...
        p_dict = self.master_.alive_players_dict()
        # 問い合わせ
        ok_send = self.player_.send_data("tell a someone's side > ", with_CR=False)
        while True:
            if not ok_send:
                sys.exit(0)
            ...
            if data.isdigit() and (int(data) in p_dict.keys()):
                ...
                if role_ is ROLES.WEREWOLF:
                    ...
                    # 白
                    side_ = 1
                ok_send = self.player_.send_data(f"tell {told_person_name}'s side\n")
                ok_send = self.player_.send_data(f"{told_person_name} is {side_list_[side_]}\n")
                return
            else:
