...
class MasterThread(Thread):
    ...
    def validate_game_condition(self):
        ...
        wolfs_ = [p for p in self.global_object.players_alive if type(
            p.role) == classes.roles.werewolf.Werewolf_Role]
        citizens_ = [p for p in self.global_object.players_alive if type(
            p.role) == classes.roles.citizen.Citizen_Role]
    ...
    def anounce_attack_result(self):
        top_user = statistics.multimode(
            self.global_object.attack_list)  # modeのlistを返す
        ...
        self.global_object.guard_list = []

    def check_game_finish(self):
        wolfs_ = [p for p in self.global_object.players_alive if type(
            p.role) == classes.roles.werewolf.Werewolf_Role]
        not_wolfs_ = [p for p in self.global_object.players_alive if type(
            p.role) != classes.roles.werewolf.Werewolf_Role]
